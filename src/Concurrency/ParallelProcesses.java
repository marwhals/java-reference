...
- Used for parallelism and concurrent execution of tasks
- Each worker has its own task queue
- When a worker finishes its own tasks and its queue is empty it can steal tasks from the back of other worker threads queues
----- The allows workload balancing on threads, reduces idle time and optimizes resource usage

The ForkJoinPool
- Javas implementation of the Work stealing pool
- Based on fork-join (Divide and conquer algorithm) -- breakdown into subtasks, compute subtasks and then combine for result

Two type of parallelism
- Task parallelism - divide program into smaller tasks that get executed concurrently. (Threads or cores)
